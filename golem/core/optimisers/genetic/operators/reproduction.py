...
from golem.core.optimisers.genetic.operators.node import GeneticPipeline, TaskStagesEnum, GeneticNode, \
    GeneticOperatorTask
...
class ReproductionController:
    ...
    def _reproduce(self, population: PopulationT, evaluator: EvaluationOperator) -> PopulationT:
        ...
        with Manager() as manager:
            task_queue, result_queue = [manager.Queue() for _ in range(2)]

            def evaluate(graphs, operator_type, evaluator=evaluator):
                individuals = [Individual(deepcopy(graph), metadata=self.mutation.requirements.static_individual_metadata)
                               for graph in graphs]
                evaluated_individuals = self.evaluator(individuals)
            ...
            worker_parameters = dict(pipeline=pipeline,
            ...
            while run:
                ...
                if not tasks:
                    try:
                        ...
                        tasks.append(self._empty_task.copy())

                # send task to pipeline
                processed_tasks = self.pipeline(tasks.pop())

                # process result
                for processed_task in processed_tasks:
                    if processed_task.stage is TaskStagesEnum.FINISH:
                        self._result_queue.put(processed_task)
                        continue
                    if processed_task.stage is TaskStagesEnum.FAIL:
                        self._log.warning(f"FAIL: {processed_task.failed_stage}")
                        self._result_queue.put(processed_task)
                        if processed_task.left_tries > 0:
