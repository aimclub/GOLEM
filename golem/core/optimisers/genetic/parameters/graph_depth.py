...
class AdaptiveGraphDepth(AdaptiveParameter[int]):
    ...
    def __init__(self, improvements: ImprovementWatcher,
        ...
        if start_depth is None or start_depth <= 0:
            raise ValueError(f'Incorrect start_depth value: {start_depth}. It should be greater than 0.')
        if max_depth is None or max_depth < start_depth:
            raise ValueError(f'Incorrect max_depth value: {max_depth}. It should be not less than start_depth.')
        if max_stagnation_gens is None or max_stagnation_gens <= 0:
            raise ValueError(f'Incorrect max_stagnation_gens value: {max_stagnation_gens}. '
